...
def main():
    print("=== 大恒 GigE 相机 ArUco 二维码 IPM 逆透视坐标识别测试 ===")
    
    # 1. 配置参数
    RESOLUTION = (2048, 1536) # 大恒相机常用分辨率
    matrix_file_path = "/home/jetson/Downloads/owl/H.npy"
    ...
    try:
        while True:
            ...
            for box in boxes:
                ...
                # 打印并显示坐标信息 (转换为 cm 显示以更直观)
                coord_text = f"ID:{int(marker_id)} R:{gx_m*100:.1f}cm F:{gy_m*100:.1f}cm"
                cv2.putText(display_frame, coord_text, (int(x1), int(y1)-10), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
                
                print(f"检测到 ArUco [{int(marker_id)}]: 像素({cx:.1f}, {cy:.1f}) -> 地面(右={gx_m*100:.1f}cm, 前={gy_m*100:.1f}cm)")

            # 显示图像 (等比例缩小到 1024x768 适应屏幕)
            display_frame_resized = cv2.resize(display_frame, (1024, 768))
            cv2.imshow("ArUco IPM Ground Detection", display_frame_resized)
